...
// Connection to db for query methods, run connect befure querying
...
const awaitDatabase = () => {
  return r.tableList().run(connection)
    ...
    .catch(() => {
      ...
      console.warn(`Retrying database in ${RETRY_WAIT / 1000} secs....`)
  ...
  return r.connect({host: HOST, port: PORT, db: NAME})
    ...
    .catch(err => {
      if (err instanceof r.Error.ReqlDriverError) {
        ...
        console.log(`Retrying in ${RETRY_WAIT / 1000} secs...`)
...
// Runs a specofoed query against a database table
...
// Use for queries that modify a table, turns error message into errors
...
// Accepts an optional validator function, which should have an erros method
